      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 bg-blue-500/20 text-blue-300 rounded-md text-xs border border-blue-500/30">
                {event.parsed_data_v8.event_type || '-'}
              </span>
            </div>
          </div>
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {event.parsed_data_v8.schemes_mentioned && event.parsed_data_v8.schemes_mentioned.length > 0 ? event.parsed_data_v8.schemes_mentioned.map(t => (
        ...
        <div className="space-y-2">
          ...
          <div className="bg-black/50 rounded-lg p-3 font-mono text-xs text-slate-400 space-y-1 border border-white/5">
            {event.feedback_log && event.feedback_log.map((log, i) => (
              <div key={i}>&gt; {log.field_name}: {JSON.stringify(log.original_value)} -&gt; {JSON.stringify(log.corrected_value)}</div>
            ))}
            {(!event.feedback_log || event.feedback_log.length === 0) && <div>&gt; No logs available</div>}
          </div>
        </div>
