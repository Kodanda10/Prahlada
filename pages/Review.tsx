...
const Review = () => {
  ...
  return (
    <div className="h-full relative pb-10 space-y-6">
      {/* Top Bar with Downloads */}
      <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5 backdrop-blur-sm">
        <h2 className="text-sm font-bold text-slate-300 font-hindi">समीक्षा क्रियाएँ (Actions)</h2>
        <div className="flex gap-2">
          <button
            onClick={handleDownloadExcel}
            className="flex items-center gap-2 px-4 py-2 bg-green-600/10 text-green-400 border border-green-500/20 rounded-xl hover:bg-green-600/20 transition-colors text-xs font-medium font-hindi"
          >
            <Download size={14} /> एक्सेल
          </button>
          <button
            onClick={handleDownloadPDF}
            className="flex items-center gap-2 px-4 py-2 bg-red-600/10 text-red-400 border border-red-500/20 rounded-xl hover:bg-red-600/20 transition-colors text-xs font-medium font-hindi"
          >
